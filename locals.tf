...
locals {
  ...
  provisioner_environment_variables = {                                                                                           // The set of environment variables set by this module on the provisioning script.
    ...
    KUBE_PROXY_NAMESPACE               = var.kube_proxy_namespace
    CILIUM_WAIT_TIMER                  = var.cilium_wait_timer // the namespace contains the kube-proxy, it should be kube-system most of the case but leave this as the var in case we found some k8s distribution use something else
  }
